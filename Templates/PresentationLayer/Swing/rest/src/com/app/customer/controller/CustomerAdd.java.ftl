...
public class CustomerAdd extends JFrame {
	...
	public void labelSetting() {
		...
		label.setIcon(new ImageIcon(CustomerAdd.class
				.getResource("/images/logo.png")));
		panel.add(label);

		label_1 = new JLabel("");
		label_1.setIcon(new ImageIcon(CustomerAdd.class
	...
	public boolean insert(Customer customer) throws JsonGenerationException,
		...
		String output = delegate.doBusiness("REST",
				"${serverHost}"+"${serviceProjectName}/"+"${Rest_URL}/ws/save", "POST",
				"", jsonInString);
	...
	public boolean update(Customer customer) throws JsonGenerationException,
			JsonMappingException, IOException {
		...
		${Validation_Call}
		String output = delegate.doBusiness("REST",
				"${serverHost}"+"${serviceProjectName}/"+"${Rest_URL}/ws/update", "POST",
